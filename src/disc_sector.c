...
//                        pclog("Find sector not found\n");
			pclog("READ: Sector (%i %i %i %i) not found (last: %i %i %i)\n", s->c, s->h, s->r, s->n, disc_sector_track, disc_sector_side, disc_sector_sector);
                        fdc_notfound();
                        disc_sector_state = STATE_IDLE;
                        break;
                }
...
//			pclog("Medium is of a density not supported by the drive\n");
		...
		if (!(fdd_can_read_medium(disc_sector_drive ^ fdd_swap)))
		{
			// pclog("Medium is of a density not supported by the drive\n");
                ...
                if (writeprot[disc_sector_drive] || swwp)
                {
                        fdc_writeprotect();
                        return;
                }
		    ...
		    disc_sector_n != s->n ||
                ...
                if (data == -1)
                        break;
                if (!disable_write)  s->data[cur_byte] = data;
                advance_byte();
                if (!cur_byte)
                {
                        disc_sector_state = STATE_IDLE;
                        if (!disable_write)  disc_sector_writeback[disc_sector_drive](disc_sector_drive, disc_sector_track);
                        fdc_finishread();
                }
                break;
		...
		if (!(fdd_can_read_medium(disc_sector_drive ^ fdd_swap)))
		{
			...
			// pclog("Bitcell period mismatch\n");
                ...
                case STATE_FORMAT:
                if (!disc_intersector_delay && fdc_get_bitcell_period() == get_bitcell_period() && !disable_write)
