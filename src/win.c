...
//                           sleep(0);
                ...
                while (GetMessage(&messages,NULL,0,0) && !quited)
                {
                        ...
                        if ((pcem_key[KEY_LCONTROL] || pcem_key[KEY_RCONTROL]) && pcem_key[0x57])
                        {
				// pclog("Taking screenshot...\n");
				take_screenshot();
                        }

                        if ((pcem_key[0xDB] || pcem_key[0xDC]) && pcem_key[0x58])
                        {
	                        pause=1;
                	        Sleep(100);
        	                resetpc_cad();
                        ...
                        if ((pcem_key[0xDB] || pcem_key[0xDC]) && pcem_key[0x57])
                        {
	                        pause=1;
                	        Sleep(100);
        	                resetpchard();
...
//                        pclog("WM_COMMAND %i\n",LOWORD(wParam));
			...
			break;
				...
				if ((old_cdrom_drive == LOWORD(wParam)-IDM_CDROM_REAL) && (cdrom_drive > 0) && (cdrom_drive <= 100) && cdrom_enabled)
				{
