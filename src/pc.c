...
//                checkkeys();
        ...
        endblit();
                ...
                {
                        win_title_update=0;
                        sprintf(s, "PCem-mooch v11.1 - %i%% - %s - %s - %s", fps, model_getname(), models[model].cpu[cpu_manufacturer].cpus[cpu].name, (!mousecapture) ? "Click to capture mouse" : "Press CTRL-END or middle button to release mouse");
                        set_window_title(s);
                }
                done++;
                frame++;
